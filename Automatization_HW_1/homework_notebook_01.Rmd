...
**сделано, но аутлаеры случайно по интерквартильному считала**
...
```{r}
sum(duplicated(cleaned_data) | duplicated(cleaned_data))
...
Костыльно, но сделано! Как сделать некостыльно - я не знаю, не смогла
...
Примечание: 95% CI для доли внутри группы получился уж очень близок к среднему за счет того, что у нас очень большое количество наблюдений. Если я все сделала правильно, то в этом случае эта метрика бессмысленна
...
```{r}
#Скопировала и сохранила сводную табличку по категориальным метрикам
cat_data <- cleaned_data %>%
  ...
  mutate(
    total = sum(count),  # Общее количество в группе для переменной
    percent = count / total,  # Относительное количество
    se = sqrt((percent * (1 - percent)) / total),
    error_margin = qt(0.975, df = total - 1) * se,
    ci_lower = round((count - error_margin), 2),
    ci_upper = round((count + error_margin), 2),
  ...
  ungroup() %>%  
  select(-total, -se, -error_margin, -ci_upper, -ci_lower) %>% # Удалила ненужные столбики
...
```{r}
ggplot(cat_data) +
  geom_col(aes(x = value, y = percent, fill = factor(cardio)), color = "black", position = "dodge") + #позиция додж чтобы столбики стояли рядом
...
cleaned_data %>%
  ...
  pivot_longer(-cardio, 
               ...
               names_sep = "_") %>%
  mutate(w_shapiro = map_dbl(shapiro, "statistic"),
         p_value = map_dbl(shapiro, "p.value")) %>% #как в одно действие распарсить значение критерия и пи велью я не знаю
  select(-shapiro)%>%
...
# этот код от чат гпт - я свой тоже написала, но попросила чат его сократить и сделать красивее
...
```{r}
cor_matrix <- cleaned_data %>% #сначала считаем матрицу корреляций
  mutate(age = as.numeric(age))%>%
  select(where(is.numeric), -id) %>%
  drop_na() %>%
  cor(use = "pairwise.complete.obs")

corrplot(cor_matrix, method = "shade", type = "lower", # потом строим плот
         addCoef.col = "grey30", diag = FALSE,         # и нааа маленьком плотууу
         cl.pos = "b", tl.col = "grey10",
         col = COL2('RdBu', 10)) 
```

Удивительно (нет), но так как ИМТ высчитан из массы тела, он сильно с ней коррелирует

Также неудивительно, что систолическое давление коррелирует с диастолическим

Никаких открытий тут нет, все довольно прозаично...

Оставим отображение только тех корреляций у кого скорректированное пи велью меньше 0,05

```{r}
corr_data <- cleaned_data %>% #делаем датасет только из количественных переменных
  mutate(age = as.numeric(age)) %>%
  select(where(is.numeric), -id) %>%
  drop_na() 

corr_p = cor.mtest(corr_data, conf.level = 0.95)

p_values <- corr_p$p[!is.na(corr_p$p)]
p_adjusted <- p.adjust(p_values, method = "bonferroni")
corr_p$p[!is.na(corr_p$p)] <- p_adjusted

## specialized the insignificant value according to the 5% significant level
corrplot(cor_matrix, p.mat = corr_p$p, sig.level = 0.05, order = 'hclust', addrect = 2)
```

А у них у всех пи велью меньше 0,05
